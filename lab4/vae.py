...
with tf.name_scope('z'):
    ...
    z_log_sigma_sq = vae_layer(layer_e2, n_hidden_recog_2, n_z, 'z_log_sigma_sq', act=tf.log)
...
x_reconstr_mean = vae_layer(layer_d2, n_hidden_gener_2, n_input, 'x_reconstr_mean')

x_reconstr_mean_out = tf.nn.sigmoid(x_reconstr_mean)

# definirajte dvije komponente funkcije cijene
with tf.name_scope('cost'):
    cost1 = -tf.reduce_sum(x*tf.log(x_reconstr_mean_out) + (1-x)*tf.log(1-x_reconstr_mean_out), axis=1)
    # cost1 = tf.nn.sigmoid_cross_entropy_with_logits(x, x_reconstr_mean_out)
