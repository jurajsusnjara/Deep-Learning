...
class SoftmaxCrossEntropyWithLogits():
    ...
    def forward(self, x, y):
        ...
        expXSum = np.sum(np.exp(x), axis=1)
        for i in range(0, num_classes):
            loss -= y[:, i] * np.log(np.exp(x[:, i]) / expXSum)
        return np.sum(loss, axis=0) / N
    ...
    def backward_inputs(self, x, y):
        ...
        N, num_classes = x.shape
        expXSum = np.hstack([np.vstack(np.sum(np.exp(x), axis=1))] * num_classes)
        grads = np.divide(np.exp(x), expXSum) - y
        return grads / N
...
class L2Regularizer():
    ...
    def forward(self):
        ...
        """
        loss = self.weight_decay / 2 * np.power(self.weights, 2)
        return np.sum(loss)
